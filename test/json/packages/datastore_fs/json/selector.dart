...
abstract class Selector {
  ...
  bool isPathInSelection(Path path) {
    ...
    for (var i=0; i<ancestors.length;i++) {
      var a = ancestors[i];
      //Skip any index paths in the list, since selectors are insensitive to lists.
      while ((i+j < pathElements.length) 
          && pathElements[i + j] is IndexPath) j++;
      if (i + j >= pathElements.length)
        return true;
      var pathElement = pathElements[i + j];
      if (!a._matchesField(pathElement.name))
        return false;
    }
    return true;
  }
   ...
   */
  bool _matchesField(String name);
...
class AnySelector extends Selector {
  ...
  bool get isPath => false;
  
  bool _matchesField(String name) => true;
...
class FieldSelector extends Selector {
  ...
  bool _matchesField(String name) => this.name == name;
...
class FieldListSelector extends Selector {
  ...
  static Selector _parseSubselections(_Tokenizer tokenizer) {
    ...
    if (tokenizer.current.type != _Token.R_PARENS) {
  ...
  bool _matchesField(String name) => fields.any((f) => f.root.name == name);
